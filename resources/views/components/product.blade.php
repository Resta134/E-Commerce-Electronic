<!-- tampipilkan produk secara keseluruhan -->
...
@endphp
<x-css />
<div class="container py-4 flex gap-2 hidden ">
    <h3 class=" text-black mb-4">Produk Terbaru dari Semua Brand</h3>

    <!-- Wrapper seluruh halaman slide -->
    <div id="product-pages">
        @php
        $chunks = array_chunk($dummyProducts, 10);
        @endphp

        @foreach ($chunks as $index => $products)
        <div class="product-slide {{ $index === 0 ? '' : 'd-none' }}">
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-5 g-4">
                @foreach ($products as $product)
                <div class="col">
                    <div class="card h-100 text-center group hover-shadow">
                        <img src="{{ $product['image'] }}" class="card-img-top hover-zoom" alt="{{ $product['name'] }}" style="height: 300px; object-fit: cover;">
    ...
    <div class="mt-4">
        <a href="/allproduct"
        class="btn btn-outline-dark">Lihat Semua Produk</a>
    </div>
